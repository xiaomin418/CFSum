...
class UniterEncoder(nn.Module):
    def __init__(self, config, mutual_layer, mutual_detach):
        super().__init__()
    ...
    def get_mutual_info(self, text_output, mutual_encoder_layer):
    ...
    def forward(self, input_, attention_mask,
        ...
        # mutual_encoder_layer = None

        def ot_guiding_attn(cross_ot_layer, attention_mask):
            cross_pool = (cross_ot_layer * attn_masks.unsqueeze(2)).sum(dim=1)
            cross_pool = cross_pool / (attn_masks.sum(dim=1).unsqueeze(1))
            txt_pool = (txt_ot_layer * attn_txt_all.unsqueeze(2)).sum(dim=1)
            txt_pool = txt_pool / (attn_txt_all.sum(dim=1).unsqueeze(1))
            ...
            # pool_diff_trunc = 0.6
            pool_diff_bool = (pool_diff > ot_trunc).long()
            pool_diff_bool = pool_diff_bool.unsqueeze(1).unsqueeze(2)
            _attn_vis_RU = attn_vis_RU * (1 - pool_diff_bool) * -10000
            b, l1, l2 = _attn_vis_RU.shape
            _attn_vis_RU = _attn_vis_RU.unsqueeze(1)
            _attn_vis_RU = _attn_vis_RU.expand(b, 12, l1, l2)
            _attention_mask = attention_mask + _attn_vis_RU
        ...
        for layer_id, layer_module in enumerate(self.layer):
            ...
            if len(present_params)!=0:
                ot_layer = present_params[0]
                if layer_id == ot_layer-1:
                    FTEST, txt_ot_layer, attn_vis_RU, attn_masks, attn_txt_all, ot_trunc = \
                        present_params[1], present_params[2], present_params[3], present_params[4], present_params[5], present_params[6]
                    if ot_trunc>=-1:
                        attention_mask = ot_guiding_attn(hidden_states, attention_mask)


            # if layer_id == self.mutual_layer-1:
            #     mutual_encoder_layer = hidden_states
...
class UniterEncoderForText(nn.Module):
    ...
    def forward(self, input_, attention_mask,
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        # if self.mutual_detach:
        #     all_encoder_layers = [layer.detach() for layer in all_encoder_layers]
        return all_encoder_layers, all_attention_scores
...
class UniterModel(UniterPreTrainedModel):
    ...
    """
    def __init__(self, config, img_dim, mutual_layer, mutual_detach):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config)
        self.img_embeddings = UniterImageEmbeddings(config, img_dim)
        self.encoder = UniterEncoder(config, mutual_layer, mutual_detach)
    ...
    def forward(self, input_ids, position_ids,
        ...
        return encoded_layers, embedding_output,all_attention_scores

class UniterModelForText(UniterPreTrainedModel):
    """ Modification for Joint Vision-Language Encoding
    """
    def __init__(self, config, img_dim, encoder_layer):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config)
        self.img_embeddings = UniterImageEmbeddings(config, img_dim)
        self.encoder = UniterEncoderForText(config, encoder_layer)
        self.pooler = BertPooler(config)
        self.apply(self.init_weights)
